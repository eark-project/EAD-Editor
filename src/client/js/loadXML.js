...
function completeInputs(){
	...
	var id = getUrlParameter("fileid");
	var sdata = { id: id};
	
	jQuery.ajax({
		type: "POST", // HTTP method POST or GET
		url: "../server/getXML.php", //Where to make Ajax calls
		data: sdata, //Form variables
		dataType: "xml",
		success:function(response){
			
			var xmlString = (new XMLSerializer()).serializeToString(response);
			parseMyXML(xmlString);
...
function SaveXML()
{
	...
	if(xmlFileName == null || xmlFileName == "") {
		...
		$.ajax({
			type: 'POST',
			url: "../server/writeXML.php",
			dataType: 'xml',
			data: {filename: xmlFileName, content: listXML},
			error: function() {
